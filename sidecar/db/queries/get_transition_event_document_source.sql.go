...
// versions:
//   sqlc v1.31.1
// source: get_transition_event_document_source.sql
...
const getTransitionEventDocumentSource = `-- name: GetTransitionEventDocumentSource :one
SELECT
...
type GetTransitionEventDocumentSourceRow struct {
...
func (q *Queries) GetTransitionEventDocumentSource(ctx context.Context, id int64) (GetTransitionEventDocumentSourceRow, error) {
	row := q.db.QueryRowContext(ctx, getTransitionEventDocumentSource, id)
	var i GetTransitionEventDocumentSourceRow
	err := row.Scan(
		&i.TransitionEventID,
		&i.ApplicationName,
		&i.Reason,
