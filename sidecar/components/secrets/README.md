...
component is three operations over someone else's store.
...
The load-bearing distinction. `keyring.ErrNotFound` is caught in all three operations and **never
reaches a caller** — `QueryGetSecret` turns it into a nil pointer, `CommandDeleteSecret` into
...
> **Note:** nothing calls this package yet. Every secret the sidecar uses today still resolves
> through the stdin handoff or the environment. Treat it as unwired but tested.
...
- **Absent is not an error.** The same stance as `QueryGetApplicationCategories` omitting an
  ...
  pointer is an answer, an `error` means the mechanism broke.
