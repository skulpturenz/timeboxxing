...
func (p *CDPPoller) URLForTitle(tabTitle string) string {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	if time.Since(p.lastPoll) >= p.minInterval {
		_ = p.fetchAndCache() // non-fatal; stale cache is fine
		p.lastPoll = time.Now()
	}
...
func (p *CDPPoller) fetchAndCache() bool {
